...
class FIR(HW):
    def __init__(self, taps):
        ...
        self.taps_fix = Const([Sfix(x, 0, -17) for x in self.taps])
    ...
    def main(self, x):
        ...
        for i in range(len(self.taps_fix)):
            self.next.mul[i] = resize(x * self.taps_fix[i], size_res=x)
            if i == 0:
                self.next.acc[0] = resize(self.mul[i], left, right_index(x))
            else:
                self.next.acc[i] = resize(self.acc[i - 1] + self.mul[i], left, right_index(x))
